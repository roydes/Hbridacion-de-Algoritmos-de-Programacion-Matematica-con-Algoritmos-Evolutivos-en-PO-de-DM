...
\begin{enumerate}[label=Paso \arabic*. , leftmargin=2cm]
...
En el experimento C se reduce el número de evaluaciones de las variantes híbridas teniendo en cuenta las gráficas de convergencia de cada algoritmo. Se realiza un ajuste de parámetros para cada variante y se mide el desempeño tanto desde el punto de vista estadístico descriptivo como inferencial.
 ...
 \subsection{Experimento A: Pruebas de estadísticas sobre el Método Nelder-Mead con Expansión de Longitud Aleatoria}\label{sec:Experimento A: Pruebas de estadísticas sobre el Método Nelder-Mead con Expansión de Longitud Aleatoria}
En el experimento A se comparan las versiones aleatorizadas NMELA, NMEILA, NM2ELA (Sección \ref{sec:NMELA}) con dos versiones deterministas del Nelder-Mead: NM y NMS (con operador de encogimiento).
 ...
 \subsubsection{Paso 2. Definición de medidas y pruebas de desempeño}\label{sec:Definición de medidas Experimento C}
 ...
 Para este experimento se utilizó un número fijo de evaluaciones para todos los algoritmos en comparación. Valores superiores a este no fueron considerados ya que debido la rápida convergencia del NM, no se evidenciaron diferencias de desempeño cuando se utilizaba un presupuesto de evaluaciones superior. En la Tabla \ref{tab:Evaluaciones utilizadas por problema de optimización: Experimento A.} se describe el número de evaluaciones empleado para cada problema:
 ...
 En la Tabla \ref{tab:resultados NMELA} se observan los resultados obtenidos para las medidas de estadística descriptivas definidas para el experimento. Las primeras dos columnas muestran los resultados correspondientes a las variantes deterministas NM (versión descrita en sección \ref{sec: Metodo NM}) y el NM con operador de encogimiento (NMS). Las columnas 3, 4 y 5 corresponden a las variantes NMELA, NMEILA y NM2ELA respectivamente.
  ...
  La Figura \ref{fig: Bonferroni-Dunn -NMELA} muestra los resultados de las pruebas post hoc de Bonferroni-Dunn para los cinco problemas de diseño cinemático. Las etiquetas 1 y 2, corresponden al NM y NMS, y las etiquetas 3, 4 y 5 corresponden a NMELA, NMEILA y NM2ELA respectivamente. Las lineas son los intervalos de confianza calculados. En caso de que dos intervalos se traslapen indica que la sub-hipótesis nula se cumple para ambos algoritmos. En lo adelante se utilizará como convención que los intervalos en rojo corresponden a los algoritmos propuestos que presentan diferencias significativas respecto al método de la literatura con que se comparan (intervalo en azul). Se dibujarán en gris aquellos intervalos que se traslapen.
  
 \begin{table}[]  
 	\centering
 	\captionsetup{width=.85\linewidth}
 	\caption[Resultados estadísticos obtenidos por las versiones aleatorizadas NMELA en el Experimento A.]{Resultados estadísticos obtenidos por las versiones aleatorizadas NMELA en el Experimento A. El i-ésimo elemento de la muestra es el valor de la función objetivo del mejor individuo factible encontrado en la ejecución $i$. Los mejores resultados se resaltan en negritas. }
...
\begin{table}
		\centering
	 	\captionsetup{width=.85\linewidth}
	\caption[Resultados estadísticos obtenidos por las variantes HNMED y DE/rand/1/bin en el Experimento B para los seis problemas de optimización.]{Resultados estadísticos obtenidos por las variantes HNMED y DE/rand/1/bin en el Experimento B para los seis problemas de optimización. El i-ésimo elemento de la muestra es el valor de la función objetivo del mejor individuo factible encontrado en la ejecución $i$. Los mejores resultados se resaltan en negritas.}\label{tab:Resultados estadísticos obtenidos por variantes HNMED y DE/rand/1/bin  experimento B.}
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{clcccccc} 
			...
			\hline
			\multirow{6}{*}{MCE1} & Mejor        & 5.5535E-28 &5.3151E-07& \textbf{0} & 1.2621E-29&5.0487E-29 &1.7670E-28 \\
			& Peor        & 2.7381E-02 & 1.6149E+00 & 2.5685E-02 & \textbf{2.2528E-02} & 2.5749E-02 & 2.7649E-02 \\
...
\subsubsection{Paso 2. Definición de medidas}
...
\begin{table}[h]
	...
	\caption{Evaluaciones utilizadas por problema de optimización: Experimento C.}
...
\begin{table}
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{clcccccc} 
			...
			& Mediana     &2.4738E-27  & 5.0487E-29   & 5.0487E-29   & 5.6797E-29   & \textbf{1.2621E-29}   & 3.9485E-23  \\
			& Promedio    &3.8030E-27   &2.3248E-28    & 4.8268E-28  & 2.4801E-28   &\textbf{ 6.2396E-29 } & 3.4957E-21  \\
			& Desv. Est.  & 4.3612E-27    &4.8191E-28   &	1.1600E-27  & 4.4014E-28   &\textbf{ 1.6612E-28}   & 1.2077E-20  \\
...
\end{figure}
\subsubsection{Paso 5. Observaciones}
Teniendo en cuenta los resultados obtenidos en el presente experimento se pueden realizar las siguientes observaciones:
\begin{enumerate}
...
\end{table}
 \newpage
\subsubsection{Paso 5. Observaciones Finales}
...
\begin{table}[hb]
	...
	\resizebox{\textwidth}{0.35\textwidth}{%	
...
\begin{table}[h]
	\centering
	\captionsetup{width=.85\linewidth}
	\caption{Vectores de diseño obtenidos por HNMED-V6 para la microrred aislada en las 24 horas del día, correspondientes al la mejor observación de una muestra de ejecuciones independientes.}\label{Vectores de diseño obtenidos por HNMED-V6 para la microrred aislada en las 24 horas del día, correspondientes al la mejor observación de la muestra}
	\centering
	\resizebox{\textwidth}{0.42\linewidth}{%	
		\begin{tabular}{|c|c|c|c|c|c|c|} 
			\hline
...
En el presente capítulo se describió el diseño experimental llevado a cabo para implementar la propuesta de solución. Este proceso de experimentación se llevo a cabo en dos etapas: una fase de experimentos preliminares que tuvieron como objetivo validar la competitividad del enfoque de hibridación propuesto y evaluar el comportamiento de las primeras cinco variantes presentadas. Los resultados obtenidos en experimentos preliminares sirvieron para guiar el diseño del algoritmo HNMED-V6 el cual incluye una estrategia de inicialización de los símpleces y el buscador global se aplica a un subconjunto élite mayor durante el proceso de minimización. Los experimentos finales muestran un desempeño competitivo de esta versión con respecto a los algoritmos ED/rand/1/bin y C-LSHADE. Sin embargo HNMED-V6 utiliza un número inferior de evaluaciones de la función objetivo.