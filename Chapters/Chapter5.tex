...
\end{equation}
Donde $e^{j\theta_1}$ es la entidad de Euler y $\theta_i$ es el angulo comprendido entre cada barra y el eje de coordenadas. La identidad de Euler plantea la siguiente relación donde $j$ es la unidad imaginaria \cite{weisstein_euler_nodate}:
\begin{equation}\label{eq:Euler}
 e^{jx}=\cos{x}+j\sin{x} 
\end{equation}


Luego sustituyendo \ref{eq:Euler} en \ref{eq:MElazoPolar} se obtiene la expresión:
...
\end{equation}
Agrupando la parte imaginaria en el miembro izquierdo y la real en el miembro derecho:
...
Sustituyendo la Ecuación \ref{eq:enFuncionTheta3} en la Ecuación \ref{eq:A1B1C1} se obtiene una ecuación lineal de segundo orden:
...
\end{eqnarray}
 En este caso se necesita minimizar la distancia entro los puntos del acoplador a los dos puntos de cada par de coordenadas. Por tanto, se plantea una función diferente y trece restricciones como se describe desde la Ecuación \ref{eq:FO_MEC3} a la Ecuación \ref{eq:g13_MEC3}:
